...
def have_update_json(item, file):
    ...
    if "versionCode" in item:
        if int(update_json.versionCode) > int(item.versionCode):
            ...
            shutil.move(file, file.parent.joinpath("{0}-{1}.zip".format(file.name.split('.')[0], item.version)))
...
def pull(json_dict: dict_, modules_folder: Path, json_file: Path, download_all=True):
